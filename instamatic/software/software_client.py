...
class SoftwareClient:
    """Simulates a Software object and synchronizes calls over a socket
    server.
    ...
    def connect(self):
        ...
        print(f'Connected to TEM/TIA server ({HOST}:{PORT})')
    ...
    def _init_dict(self):
        from instamatic.TEMController.software import get_software
        sw = get_software(self.name)

        self._dct = {key: value for key, value in sw.__dict__.items() if not key.startswith('_')}
